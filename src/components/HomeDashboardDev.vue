...
export default {
	...
	data ()  {
		...
		headcount_separated_option: {
			...
			labels: ['Inside Cleanroom', 'Not in cleanroom','Absent'],
		},
		in_company: '',
		cleanroom: [17,6],
		cleanroom_inroom:[{
	...
	methods: {
		async man_power_plan() {
			try{
				...
				// const Transa = await axios.get("http://43.239.251.75:8000/api/Transaction");
				// console.log("TRA",Transa.data);
		...
		Head_CO(apidata){
			...
			this.headcount_separated = [InsideCleanroomCount, NotInCleanroomCount, AbsentCount]; //GoHomeCount
	...
	// 	categorizeEmployees(apidata) {
    //     for (const DATA of apidata) {
    //         // ตรวจสอบ process ในข้อมูลและสร้างโครงสร้างสำหรับแต่ละ process ที่พบ
    //         let process = DATA.employeeInfo.process;
    //         if (!this.departments[process]) {
    //             this.$set(this.departments, process, {
    //                 IN_COMPANY: [],
    ...
    //         // ตรวจสอบว่าพนักงานมีการมาทำงานหรือไม่
    //         let attendanceStatus = DATA.manpowerPlan.attendance === 'O' ? 'IN_COMPANY' : 'NEED_ALLOWCATE';
    //         // ตรวจสอบว่ามีข้อมูลของพนักงานในแผนกนี้หรือไม่
    //         let employeeInfo = {
    //             empID: DATA.employeeInfo.empID,
    //             firstName: DATA.employeeInfo.firstName,
    //             lastName: DATA.employeeInfo.lastName
    ...
    //         // เพิ่มข้อมูลพนักงานลงในแผนกที่เหมาะสม
    //         this.departments[process][attendanceStatus].push(employeeInfo);
    //     }
	// 	this.require_support_option= {
	// 				colors : ['#5598FE', '#BB3939'],
	...
	// 			console.log("IN_COMPANY:", this.departments[departmentName].IN_COMPANY.length);
	// 			console.log("NEED_ALLOWCATE:", this.departments[departmentName].NEED_ALLOWCATE.length);
	// 			this.req1.push(departmentName);
	// 			this.require_support[0].data.push(this.departments[departmentName].IN_COMPANY.length);
	// 			this.require_support[1].data.push(this.departments[departmentName].NEED_ALLOWCATE.length);
	// 		}		
